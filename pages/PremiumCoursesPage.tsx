...
import { ModalType, Testimonial } from '../types';
import { useRouter } from '../hooks/useRouter';
import CountdownTimer from '../components/CountdownTimer';
import { FaTelegram, FaCheckCircle } from 'react-icons/fa';
import { FiChevronLeft, FiChevronRight, FiArrowRight, FiArrowLeft } from 'react-icons/fi';
...
const TestimonialCarousel: React.FC<{ testimonials: Testimonial[] }> = ({ testimonials }) => {
    ...
    return (
        <div className="relative w-full max-w-2xl mx-auto h-64">
            ...
            <button onClick={prevSlide} className="absolute top-1/2 -left-4 md:-left-10 transform -translate-y-1/2 bg-white/50 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-900 rounded-full p-2 z-10"><FiChevronLeft size={24} /></button>
            <button onClick={nextSlide} className="absolute top-1/2 -right-4 md:-right-10 transform -translate-y-1/2 bg-white/50 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-900 rounded-full p-2 z-10"><FiChevronRight size={24} /></button>
        </div>
    );
};
