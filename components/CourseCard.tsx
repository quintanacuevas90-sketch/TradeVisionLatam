...
const CourseCard: React.FC<CourseCardProps> = ({ course, isFeatured = false, onClose }) => {
    ...
    const isBeca = course.price === "BECA";
    const isPdf = course.link.toLowerCase().endsWith('.pdf');
    ...
    // Configuración dinámica de atributos del enlace
    const linkProps = (isInternalPath || isActionTrigger)
        ? { href: course.link, onClick: handleClick } 
        : { 
            href: course.link, 
            // Si es PDF, eliminamos target="_blank" para que la descarga sea directa
            ...(isPdf ? {} : { target: "_blank", rel: "noopener noreferrer" }),
            ...(isPdf ? { download: "Cerebro_Digital_TradeVision.pdf" } : {})
          };
    ...
    return (
        <div className={`
            ...
            ${isBeca ? 'border-brand-accent/40 bg-gradient-to-b from-[#0A1931]/80 to-gray-800/50' : ''}
        `}>
            ...
            <div className="my-4 min-h-[80px] flex flex-col justify-center">
                {isBeca ? (
                    <div className="py-2">
                        <span className="inline-block bg-brand-accent/20 text-brand-accent border border-brand-accent/40 px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-[0.2em] shadow-[0_0_15px_rgba(64,224,208,0.15)]">
            ...
            <p className={`text-xs font-bold py-1.5 px-4 rounded-full my-2 self-center ${isBeca ? 'bg-brand-accent/10 text-brand-accent border border-brand-accent/20' : 'bg-red-600 text-white'}`}>
                {course.urgency}
            </p>
            ...
            >
                {course.cta} {isBeca ? <FiDownload /> : <FiArrowRight />}
            </a>
        </div>
    );
};
